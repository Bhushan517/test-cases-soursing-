...
export async function createOffer(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    const jobId = jobs.id
    if (userTypes === "vendor") {
      await checkDistributionStatusForVendor(program_id, userTypes, jobId, userData, reply, traceId)
    }
    ...
    if (!offerData.financial_details?.rates) {
      return reply.status(400).send({ message: "Financial details cannot be null" });
    }

    if (!offerData.financial_details?.fee_details) {
      return reply.status(400).send({ message: "Fee details cannot be null" });
    }
    ...
    if (!jobDatas) {
      return reply.status(400).send({ massage: "Job not found for the provided job ID." });
    }
    ...
    const existingOffer = await validateExistingOffer(offerData);
    ...
    if (newItem?.onboarding_flow_id) {
      processOnboardingFlow(jobDatas, newItem, program_id, request, transaction, traceId, reply, hierarchyIds);
    }
    ...
    try {
      await processCustomFields(newItem.id, offerData.custom_fields, transaction);
      await processFoundationalData(newItem.id, offerData.foundational_data, transaction);
...
async function processOnboardingFlow(job: { job_id: any; id: any; job_template_id: string; }, offer: OfferModel, program_id: string, request: FastifyRequest, transaction: Transaction, traceId: string, reply: FastifyReply, hierarchy_ids: string[]) {
  ...
  try {
    ...
    try {
      ...
      return reply.status(500).send({
        status_code: 500,
        trace_id: traceId,
        message: "Failed to add onboarding steps",
        error: error.message,
      });
    }
  } catch (error: any) {
...
async function validateExistingOffer(offerData: OfferInterface) {
  let offer;
